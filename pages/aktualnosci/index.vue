<template>
	<div>
		...
		<v-container class="mt-8">
			<v-row  v-for="aktualnosc in aktualnosci" :key="aktualnosc.id">
				<v-col cols="12" md="12" class="pa-0 ma-0 order-1 order-md-0">
					<CardComponent 
						:isExtended="true"
						:isNews="true"
						:isPerson="false"
						:title="aktualnosc.attributes.Title"
						:featured_photo="apiUrl+aktualnosc.attributes.Image.data.attributes.url"
						:text="aktualnosc.attributes.Excerpt"
						:date="aktualnosc.attributes.Date"
					/>
				</v-col>
			</v-row>
		</v-container>
...
export default {
	...
	async mounted () {
		try {
			const responseAktualnosci = await axios.get(this.apiUrl+'/api/aktualnoscs?populate=Image&sort=id:desc')
